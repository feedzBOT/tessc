...
stiker = await sticker(false, "https://telegra.ph/file/dfee29786ff4c524443c2.png", "Botti-BOT", "Ngetag lagi saya banned")
	...
	if (!m.fromMe && m.text.match(/(🙂|😭|🙃)/gi)) {
		...
		var warning = global.DATABASE.data.users[m.sender].warning
			if(warning > 4) {
				conn.reply(m.chat, `*hamdeeh sorry kawand!*`, m).then(() => {
				conn.groupRemove(m.chat, [m.sender])
				global.DATABASE.data.users[m.sender].warning = 0
           	 })
			} else {
	...
	if(enable.nobomchat && !m.fromMe && m.isGroup && isBotAdmin) {
		...
		var warning = global.DATABASE.data.users[m.sender].warning
			if(warning > 9) {
				conn.reply(m.chat, `*jangan spam bangsat!*`, m).then(() => {
				conn.groupRemove(m.chat, [m.sender])
				global.DATABASE.data.users[m.sender].warning = 0
           	 })
			} else {
